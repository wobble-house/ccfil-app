...
import { getFeatureFeedback } from "../utils/graphql/queries";
import { updateFeatureFeedback } from "../utils/graphql/mutations";
const client = generateClient();
export default function FeatureFeedbackUpdateForm(props) {
  const {
    id: idProp,
    featureFeedback: featureFeedbackModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = featureFeedbackRecord
      ? { ...initialValues, ...featureFeedbackRecord }
      : initialValues;
    setDate(cleanValues.date);
    setLayoutDesign(cleanValues.layoutDesign);
    setFunctionality(cleanValues.functionality);
    setFormFields(cleanValues.formFields);
    setFeatureRequest(cleanValues.featureRequest);
    setOther(cleanValues.other);
    setIsComplete(cleanValues.isComplete);
    setErrors({});
  };
  const [featureFeedbackRecord, setFeatureFeedbackRecord] = React.useState(
    featureFeedbackModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getFeatureFeedback.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getFeatureFeedback
        : featureFeedbackModelProp;
      setFeatureFeedbackRecord(record);
    };
    queryData();
  }, [idProp, featureFeedbackModelProp]);
  React.useEffect(resetStateValues, [featureFeedbackRecord]);
  const validations = {
    date: [],
    layoutDesign: [],
    functionality: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateFeatureFeedback.replaceAll("__typename", ""),
            variables: {
              input: {
                id: featureFeedbackRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "FeatureFeedbackUpdateForm")}
      {...rest}
    >
      <TextField
        label="Date"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || featureFeedbackModelProp)}
