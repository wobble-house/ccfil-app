...
export default function Nav({mini}:{mini:boolean}) {
  ...
  return (
      <nav className={`z-90 flex md:justify-center w-full md:w-auto right-0 top-0 md:-mt-4 pb-4 ${mini == true ? "fixed bg-white bg-opacity-75 mr-auto pl-4 rounded-md ":""} `}>
        <div className="flex flex-col-reverse md:flex-row flex-grow justify-end">
                <button className="fixed z-90 md:hidden bg-white px-2 border-2 rounded-md border-blue1 focus:border-2 focus:animate-pulse right-0 top-0 mr-5 mt-4" onMouseDown={handleMainNavButton}>
                  {navbar ? (
            ...
            <div ref={Navref} className={`relative flex mx-auto md:mx-0 bg-blue1 dark:bg-blue2 md:bg-transparent md:block  ${navbar ? 'fixed z-80 top-0 flex flex-col flex-grow min-h-content items-center w-full justify-center py-32' : 'hidden'}`}>
