...
export function NavHeader({description, title, metadata}){
  ...
  useOnClickOutside(Navref, handleNavClickOutside)
   return(
      <div className="flex z-40 md:justify-center top-0 overflow-hidden">
        ...
        <div className={`flex flex-col md:px-16 md:pb-6 bg-white bg-opacity-25 md:bg-opacity-75 w-full md:w-3/4 z-10 md:m-12 ${pathname == '/' ? 'md:min-h-[600px]' : ''}`}>
          ...
          {pathname != "/" ?
            ""
          :
            <div className="absolute bottom-20 w-2/3 flex flex-row justify-between pb-20 px-4 md:px-0 mx-auto bg-white md:bg-transparent bg-opacity-50">
